...
import { SickLeaveData } from "../../../data/AttendanceData";
...
export default function TableSickLeave() {
  ...
  const filteredSickLeave = SickLeaveData.filter((employee) => {
  ...
  // Pagination
  const totalPages = Math.ceil(filteredSickLeave.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedEmployees = filteredSickLeave.slice(
    startIndex,
  ...
  const handleEditSickLeave = (id) => {
    setSickLeave(id, SickLeaveData);
    ...
    <div className="bg-white rounded-2xl py-4 lg:py-6 shadow-sm border border-gray-100">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center lg:justify-between gap-4 mb-6">
        <div className="flex items-center gap-4 px-4 lg:px-6">
          <h2 className="text-lg font-medium">Daftar Sakit</h2>
        </div>
      ...
      <div className="overflow-x-auto">
        <div className="space-y-4">
              ...
              {paginatedEmployees.length > 0 ? (
                paginatedEmployees.map((employee) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      {format(employee.tanggalIzinSakit, "dd/MM/yyyy")}
                    </TableCell>
                    <TableCell className="text-center">
                      {employee.status}
                    </TableCell>
                    <TableCell>
                      <Button
                        ...
                        title="Edit"
                        onClick={() => handleEditSickLeave(employee.id)}
                      >
                        <span>
                          <NotePencil size={19} />
                        </span>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    <div className="flex flex-col items-center gap-2">
                      <p className="font-medium">Belum ada data sakit.</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
          ...
          <ConfirmModal
            ...
            title="Tambah Pengajuan Sakit Berhasil"
            description="Pengajuan Sakit berhasil ditambahkan."
            onClick={() => {
              setIsSecondModalOpen(false);
              resetForm();
            }}
          />
          <EditSickLeaveModal sickLeave={SickLeaveData} />
          <ConfirmModal
            ...
            title="Edit Pengajuan Sakit Berhasil"
            description="Pengajuan Sakit berhasil diubah."
            onClick={() => {
              setIsEditSecondModalOpen(false);
              resetEditForm();
            }}
