...
import TableSickLeave from "../table/TableSickLeave";
import TablePermissionLeave from "../table/TablePermissionLeave";
import TableUnpaidLeave from "../table/TableUnpaidLeave";
...
export default function PermissionAndLeave() {
  ...
  return (
    <div className="flex flex-col gap-9">
      {/* Option Button */}
      <div className="relative w-full xl:w-3/4 2xl:w-1/2 h-6 sm:h-8 lg:h-10 bg-[#F5F5F5] rounded-lg p-1 flex items-center font-medium text-[#455468]">
        <div
        ...
        {["Cuti Tahunan", "Cuti Khusus", "Sakit", "Izin", "Unpaid Leave"].map(
          (label, index) => (
            <button
              ...
              className={`z-10 w-1/5 h-full flex items-center justify-center rounded-lg transition-colors duration-300 text-[7px] sm:text-sm md:text-base lg:text-sm`}
            >
              {label}
            </button>
          )
